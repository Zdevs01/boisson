        ...
        <div class="container-fluid">
            <table class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                    <?php 
                    $i = 1;
                    // Filtrer les ventes où le mode de vente est 'livraison'
                    $qry = $conn->query("SELECT * FROM `sales_list` WHERE `mode_vente` = 'livraison' ORDER BY `date_created` DESC");
                        ...
                        <td align="center">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item text-dark" href="<?php echo base_url.'admin?page=Livreurs/view_sale&id='.$row['id'] ?>">
...
<?php 
// Récupérer les données de vente par mois pour la statistique
$sales_data = [];
$labels = [];
$amounts = [];

$qry = $conn->query("SELECT DATE_FORMAT(date_created, '%b %Y') as month, SUM(amount) as total FROM sales_list WHERE mode_vente = 'livraison' GROUP BY month ORDER BY date_created ASC");
while ($row = $qry->fetch_assoc()) {
    $labels[] = $row['month'];
    $amounts[] = $row['total'];
}
