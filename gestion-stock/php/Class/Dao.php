...
class Dao {
    ...
    public static function ajouterPersonne($nom, $prenom, $adr, $tele, $email, $image, $role, $nom_de_class) {
        $pdo = Dao::getPDO();
        $sql = "INSERT INTO {$nom_de_class}(nom,prenom,adr,tele,email, image, role) VALUES (?,?,?,?,?,?,?)";
        $pdo->prepare($sql)->execute([$nom, $prenom, $adr, $tele, $email, $image, $role,]);
    }

    // Modifier un Client ou Fournisseur
    public static function modifierPersonne($id, $nom, $prenom, $adr, $tele, $email, $image, $role, $nom_de_class) {
        $pdo = Dao::getPDO();
        $sql = "UPDATE {$nom_de_class} SET nom=?, prenom=?, adr=?, tele=?, email=?, image=?, role=? WHERE id=?";
        $pdo->prepare($sql)->execute([$nom, $prenom, $adr, $tele, $email, $image, $role, $id]);
        // print_r($pdo);
    }
    ...
    public static function ajouterAdmin($nom, $prenom, $adr, $tele, $email, $mdp, $image, $role, $nom_de_class = "Admin") {
        $pdo = Dao::getPDO();
        $sql = "INSERT INTO {$nom_de_class} (nom, prenom, adr, tele, email, mdp, image, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        if ($stmt->execute([$nom, $prenom, $adr, $tele, $email, $mdp, $image, $role])) {
            return true;
        } else {
            return false;
        }
...
// Modifier un Admin
public static function modifierAdmin($id, $nom, $prenom, $adr, $tele, $email, $mdp, $role, $nom_de_class = "Admin") {
    $pdo = Dao::getPDO();
    $sql = "UPDATE {$nom_de_class} SET nom=?, prenom=?, adr=?, tele=?, email=?, mdp=?, role=? WHERE id=?";
    $pdo->prepare($sql)->execute([$nom, $prenom, $adr, $tele, $email, $mdp, $role, $id]);
}
// Modifier l'image d'un admin
public static function modifierImageAdmin($id, $image, $nom_de_class = "Admin") {
    $pdo = Dao::getPDO();
    $sql = "UPDATE {$nom_de_class} SET image=? WHERE id=?";
...
public static function modifierMarque($nom_marque, $description_marque, $image, $id_marque) {
    try {
        ...
        $_SESSION['message'] = ['type' => 'success', 'content' => 'Marque modifiée avec succès.'];
    } catch (Exception $e) {
        // Si la modification échoue
        $_SESSION['message'] = ['type' => 'error', 'content' => 'Erreur lors de la modification de la marque.'];
    }
}
